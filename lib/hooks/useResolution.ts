...
import { SELECTED_VIDEO_QUALITY } from '@/lib/constant/texts';
...
export default function useResolution() {
  ...
  const switchQuality = useCallback(
    async ({ h, firstTime }: { h: number | string; firstTime?: boolean }) => {
      ...
      try {
        ...
        if (playerName === 'shaka' && window.shakaPlayer) {
          ...
          if (isNaN(height)) {
            ...
            const activeTrack = playlist.find((x: any) => x.active);
            const matchedTrack = playlist.find(
              (t: any) =>
                t.height === height &&
                t.language === activeTrack.language &&
                t.audioId === activeTrack.audioId, // keep same audio
            );
            await wait({ time: 300 });
            window.shakaPlayer.configure({
              abr: {
            ...
            if (matchedTrack) {
              window.shakaPlayer.selectVariantTrack(matchedTrack, false);
            } else {
              window.shakaPlayer.configure({
                abr: {
                  restrictions: {
