...
const NextRecommend: React.FC<NextRecommendProps> = ({ isExpanded }) => {
  ...
  const isHideImg = useMemo(() => {
    return width >= 768 && width < 1024 && !isExpanded;
  }, [width, isExpanded]);
      ...
      <div
        className={`w-fit flex flex-col lg:flex-row xl:flex-row lg:items-end xl:items-end gap-4 lg:gap-6 xl:gap-8 absolute bottom-[20px] tablet:bottom-[42px] lg:bottom-[36px] xl:bottom-10 ${
          isExpanded
            ...
            : 'left-[16px] tablet:left-[32px] lg:left-[32px] xl:left-[32px] right-[16px] tablet:right-[32px] lg:right-[32px] xl:right-auto xl:max-w-[1216px]'
        }`}
      >
        {/* Image */}
        <div
          className={`${
            isHideImg ? 'hidden' : 'hidden lg:block xl:block'
          } w-full lg:w-[320px] xl:w-[416px] h-[120px] tablet:h-[140px] lg:h-[180px] xl:h-[234px] bg-cover bg-center rounded-2xl flex-none order-0 z-0`}
        >
          <div
        ...
        {/* Content Container */}
        <div
          className={`flex flex-col items-start p-0 gap-2 tablet:gap-3 lg:gap-4 lg:mr-[32px] xl:gap-8 h-auto lg:h-[180px] xl:h-[232px] flex-none order-1 flex-grow z-[1] w-full lg:w-auto xl:w-auto lg:flex-1 xl:flex-1 min-w-0`}
        >
          {/* Info Section */}
          <div
            className={`flex flex-col items-start p-0 gap-2 tablet:gap-3 lg:gap-3 xl:gap-4 w-full h-auto lg:h-[120px] xl:h-[152px] flex-none order-0 self-stretch min-w-0`}
          >
            {/* Title */}
            <div className="flex justify-between w-full min-w-0">
              <h2
                className={`flex-1 h-auto font-semibold text-[18px] tablet:text-[22px] lg:text-[24px] xl:text-[28px] leading-[130%] tracking-[0.02em] text-white overflow-hidden text-ellipsis whitespace-nowrap min-w-0`}
              >
          ...
          {/* Buttons Section */}
          <div
            className={`flex flex-row items-center p-0 gap-3 tablet:gap-4 lg:gap-3 xl:gap-4 w-full h-auto flex-none order-1 min-w-0`}
          >
            {hasTrailer ? (
              <>
                {/* Watch Now Button - No animation when trailer exists */}
                <button
                  className={`flex items-center justify-center cursor-pointer px-3 tablet:px-4 lg:px-4 xl:px-6 py-2 lg:py-2 xl:py-3 gap-2 w-[120px] tablet:w-[140px] lg:w-[140px] xl:w-[178px] h-[36px] tablet:h-[38px] lg:h-[38px] xl:h-12 rounded-[40px] bg-gradient-to-r from-fpl to-red-600 hover:from-fpl-dark hover:to-red-700 transition-all ${styles.focusRing} flex-shrink-0`}
                  onClick={onWatchNow}
                  aria-label="Xem ngay"
                >
                  <div className="w-5 h-5 tablet:w-6 tablet:h-6 lg:w-6 lg:h-6 flex items-center justify-center">
                    <FaPlay className="w-3 h-3 tablet:w-3.5 tablet:h-[18px] lg:w-3.5 lg:h-[18px] text-white-smoke" />
                  </div>
                  <span className="font-semibold text-xs tablet:text-sm lg:text-sm xl:text-base leading-[130%] tracking-[0.02em] text-white-smoke whitespace-nowrap">
                    Xem ngay
                  </span>
                </button>
                ...
                <button
                  className={`relative cursor-pointer flex items-center justify-center px-3 tablet:px-4 lg:px-4 xl:px-6 py-2 lg:py-2 xl:py-3 gap-2 w-[140px] tablet:w-[160px] lg:w-[160px] xl:w-[197px] h-[36px] tablet:h-[38px] lg:h-[38px] xl:h-12 rounded-[40px] bg-charleston-green/80 hover:bg-charleston-green transition-all overflow-hidden ${styles.focusRing} flex-shrink-0`}
                  onClick={onWatchTrailer}
                  aria-label="Phát trailer"
                >
                  <div className="relative z-2 w-5 h-5 tablet:w-6 tablet:h-6 lg:w-6 lg:h-6 flex items-center justify-center">
                    <FaPlay className="w-3 h-3 tablet:w-3.5 tablet:h-[18px] lg:w-3.5 lg:h-[18px] text-white-smoke" />
                  </div>
                  <span className="relative z-2 font-semibold text-xs tablet:text-sm lg:text-sm xl:text-base leading-[130%] tracking-[0.02em] text-white-smoke whitespace-nowrap">
                    {countdown > 0
              ...
              <button
                className={`relative cursor-pointer flex items-center justify-center px-4 tablet:px-5 lg:px-5 xl:px-6 py-2 lg:py-2 xl:py-3 gap-2 w-[120px] tablet:w-[140px] lg:w-[140px] xl:w-[178px] h-[36px] tablet:h-[38px] lg:h-[38px] xl:h-12 rounded-[40px] bg-charleston-green-08 hover:bg-charleston-green transition-colors duration-200 overflow-hidden ${styles.focusRing} ${styles.button} flex-shrink-0`}
                onClick={onWatchNow}
                aria-label="Xem ngay"
                type="button"
              >
                ...
                {/* Play Icon */}
                <div className="relative z-2 w-5 h-5 tablet:w-6 tablet:h-6 lg:w-6 lg:h-6 flex items-center justify-center">
                  <FaPlay className="w-3 h-3 tablet:w-[14px] tablet:h-[14px] lg:w-[14px] lg:h-[14px] text-white-smoke" />
                </div>
                <span className="relative z-2 font-semibold text-xs tablet:text-sm lg:text-sm xl:text-base leading-[130%] tracking-[0.02em] text-white-smoke whitespace-nowrap">
                  Xem ngay
                </span>
