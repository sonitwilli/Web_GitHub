...
import { MESSAGE_NO_CONNECTION, DEFAULT_ERROR_MSG, NUMBER_PR, TYPE_PR, DETAIL_PR } from '@/lib/constant/texts';
...
interface Cookie {
  set: (key: string, value: string, options?: { path?: string; maxAge?: number }) => void
  remove: (key: string) => void
}

export const saveProfile = ({ cookie, profile }: { cookie?: Cookie; profile?: Profile } = {}): void => {
  if (typeof localStorage !== 'undefined') {
  ...
  if (cookie && cookie.set) {
    if (profile?.profile_id) {
      cookie.set(NUMBER_PR, profile.profile_id, {
        path: '/',
        maxAge: 3600 * 24 * 365, // 1 năm
      })
    }
    if (profile?.profile_type) {
      cookie.set(TYPE_PR, profile.profile_type, {
        path: '/',
        maxAge: 3600 * 24 * 365, // 1 năm
      })
    }
...
export const convertMsg = ({ msg = '', text_format = [] }: ConvertMsgParams = {}): string => {
  try {
    if (msg) {
